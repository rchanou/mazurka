...
import { chan, go, putAsync, timeout } from 'js-csp';
...
/*const xInput = new Funnel();
const yInput = new Funnel();
const sumTest = plumb( (a,b)=>a+b, [xInput,yInput] );
plumb( console.log.bind(this), ['sum', sumTest] )
xInput.push(11); xInput.push(13);*/
...
//plumb( console.log.bind(this), [pow3] );
//plumb( console.log.bind(this), ['als', pow3] );
...
const wire = (fn, ...args) => {
  ...
  go(function*(){
    while (true){
      ...
      putAsync( out, fn.apply(0, newArgs) );
    }
    //console.log(3, newArgs);
  });
  return out;
}
...
const pipe = (fn, ...args) => {
  ...
  return new Function(...argChars.filter( (v,i) => args[i] === B.Input ), funcCode);
};
...
const buildNodeTree = (prog, key) => prog[key].map(
  token => {
    if (!Array.isArray(token) || token[0] !== B){
      return token;
    }
...
const getMaxDepth = node => Array.isArray(node)? (
  1 + Math.max( ...node.map( n => getMaxDepth(n) ) )
): 0;

const elWidth = 35; const yUnit = 45;
const borderColor = 'steelblue';
const selectedColor = 'hsl(120,70%,85%)';

const renderTree = (tree, bounds, mods) => { // TODO: merge bounds into mods
...
const renderNodes = (nodes, bounds) => {
  ...
  for (const key in nodes){
    ...
    if (tree.from){
      lines.push(
        <line key={tree.nodeId + 'L'}
          x1={(nodes[tree.from].x + 0.5) * elWidth} y1={bounds.y - (nodes[tree.from].y + 1.32) * yUnit}
          x2={(tree.x + 0.5) * elWidth} y2={bounds.y - (tree.y + 0.67) * yUnit}
          strokeWidth={3} stroke={tree.selected? 'tomato': `steelblue`}
        />
      );
    }
    ...
    els.push(
      <g key={tree.nodeId}>
        <circle fill='white' stroke={tree.selected? selectedColor: borderColor}
...
const renderView = (bounds, prog, key) => {
  ...
  saveNavKeys(tree); console.log('tre', tree)
  const els = renderTree(tree, bounds);

  return <svg width={bounds.x} height={bounds.y} style={{ background: '#ccc' }}>
    {els}
  </svg>;
}
...
const encodeToken = token => (Array.isArray(token) && token[0] === B)? `${typeCodes.start}R${typeCodes.end}${token[1]}`:
  typeof token === 'number'? `${typeCodes.start}N${typeCodes.end}${token}`:
  typeof token === 'string'? `${typeCodes.start}S${typeCodes.end}${token}`:
  typeof token === 'boolean'? `${typeCodes.start}B${typeCodes.end}${token}`:
  token === undefined? `${typeCodes.start}U${typeCodes.end}`:
  token === null? `${typeCodes.start}O${typeCodes.end}null`:
...
const mapLayoutTree = (token, prog, baseX = 0, baseY = 0) => {
  ...
  const subMapLayoutTree = (token, prog, baseX, baseY, nodePath) => {
    if (!Array.isArray(token) || token[0] !== B){
      return {
        id: token, nodeId: getUniqueId(nodePath),
        x: baseX, y: baseY, width: 1
      };
    }
    ...
    if (reserved[keyword]){
      return {
        id: keyword, nodeId: getUniqueId(nodePath),
        x: baseX, y: baseY, width: 1
      };
    }
    ...
    for (const stringI in fromLine){
      ...
      const kid = subMapLayoutTree(kidToken, prog, currentKidX, kidY, [encodeToken(kidToken), ...nodePath]);
...
const cache = {
  ...
  names: {
    0: '➗',
    1: <tspan textAnchor='middle' alignmentBaseline='middle'>
      x
      <tspan baselineShift='super' style={{ fontSize: 11 }}>y</tspan>
    </tspan>,
    2: '√',
    3: 'w.console',
    4: 'c.log',
    5: <tspan textAnchor='middle' alignmentBaseline='middle'>
      x
      <tspan baselineShift='super' style={{ fontSize: 11 }}>2</tspan>
    </tspan>,
    6: <tspan textAnchor='middle' alignmentBaseline='middle'>
      y
      <tspan baselineShift='super' style={{ fontSize: 11 }}>2</tspan>
    </tspan>,
    7: <tspan textAnchor='middle' alignmentBaseline='middle'>
      3
      <tspan baselineShift='super' style={{ fontSize: 11 }}>2</tspan>
    </tspan>,
    8: <tspan textAnchor='middle' alignmentBaseline='middle'>
      4
      <tspan baselineShift='super' style={{ fontSize: 11 }}>2</tspan>
    </tspan>,
    9: <tspan textAnchor='middle' alignmentBaseline='middle'>
      3
      <tspan baselineShift='super' style={{ fontSize: 11 }}>2</tspan>
      + 4
      <tspan baselineShift='super' style={{ fontSize: 11 }}>2</tspan>
    ...
    10: 'hyp',
    16: <tspan textAnchor='middle' alignmentBaseline='middle'>
      z
      <tspan baselineShift='super' style={{ fontSize: 11 }}>2</tspan>
    </tspan>,
    17: 'c',
...
const hypotenuse = parse(rawHypotenuse);
window.h = hypotenuse;
